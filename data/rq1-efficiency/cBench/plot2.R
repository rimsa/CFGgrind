...
process <- t(as.matrix(data[,if (callgrind) c(6,5,4,8) else c(6,5,8)]));
...
colors <- c("#e21877", "#458a26", if (callgrind) "#f95c3a", "#3414ad");
ptypes <- c(19, if (callgrind) 19, 19, 19);
...
if (!simple) {
    modes <- c("DCFG", "bfTrace", if (callgrind) "callgrind", "CFGgrind");

    mtext(text="Slowdown", at=0.60, side=2, line=-2, outer=TRUE);
    text(1:length(data$benchmark), 4, srt=45, adj=1, labels=colnames(process), xpd=T, cex=1.2, offset=10);

    legend(length(data$benchmark)+1.65, max(process)/5, modes, border=T, pch=ptypes, col=colors, cex=1, pt.cex=1.5, xpd=NA);
}
